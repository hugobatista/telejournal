...
def process_message(msg_div: Tag) -> dict[str, Any]:
    ...
    classes = classes_raw if isinstance(classes_raw, list) else [classes_raw] if classes_raw else []
    ...
    for child in msg_div.children:
        ...
        child_classes_list = child_classes if isinstance(child_classes, list) else [child_classes] if child_classes else []
        if child.name == 'div' and 'attachment' not in child_classes_list and 'msg-quote' not in child_classes_list and 'footer' not in child_classes_list and 'edited' not in child_classes_list:
...
def convert_html_to_markdown(html_path: str, output_dir: str) -> int:
    ...
    for element in conversation_box.children:
        ...
        if isinstance(element, Tag) and element.name == 'div':
            ...
            element_classes_list = element_classes if isinstance(element_classes, list) else [element_classes] if element_classes else []
        ...
        for msg_data in messages:
            if msg_data['type'] == 'status':
                ...
                if msg_data['timestamp']:
                    markdown_lines.append(f' %% {format_timestamp(msg_data["timestamp"])} %%')
                markdown_lines.append('\n\n')
                ...
                for att in msg_data['attachments']:
                    ...
                    if attachment_key not in copied_attachments:
                        ...
                        if os.path.exists(source_path):
                            ...
                            dest_path = os.path.join(year_attachments_dir, filename_only)
                            try:
                                shutil.copy2(source_path, dest_path)
                                copied_attachments[attachment_key] = filename_only
                                attachments_per_year[date_obj.year].add(filename_only)
                                print(f"  Copied to {date_obj.year}/attachments/: {filename_only}")
    ...
    total_attachments: int = sum(len(att_set) for att_set in attachments_per_year.values())
    print(f"  {total_attachments} attachments copied across {len(attachments_per_year)} year folders")
